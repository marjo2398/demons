...
if (!isset($_SESSION['is_host']) || $_SESSION['is_host'] !== true) {
    ...
    <div class="bg-slate-900 border border-slate-800 p-8 rounded-2xl border border-slate-800 shadow-2xl shadow-2xl max-w-sm w-full">
        <h1 class="text-2xl font-bold text-indigo-500 mb-6 text-center"><?= t('login_title') ?></h1>
        <?php if($error): ?><div class="bg-indigo-900/50 text-indigo-200 p-3 text-sm rounded mb-4 text-center border border-indigo-900 font-bold">⚠️ <?= $error ?></div><?php endif; ?>
        <form method="POST">
...
?>
    ...
    <nav class="bg-slate-900 border-b border-slate-800 p-4 sticky top-0 z-50 shadow-lg">
        <div class="container mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <h1 class="text-xl font-bold text-indigo-500 tracking-wider flex items-center gap-2">🛡️ <?= t('host_panel') ?></h1>
            <div class="flex flex-wrap gap-2 text-sm items-center justify-center">
                ...
                <a href="index.php" class="px-3 py-1 rounded border border-slate-700 hover:bg-slate-800 text-indigo-500 font-bold transition"><?= t('back') ?></a>
    ...
    <main class="container mx-auto p-4 flex-grow">
        ...
        <?php endif; ?>
        <?php if($error): ?><div class="bg-indigo-900/50 text-indigo-200 p-3 rounded mb-6 text-center border border-indigo-900 font-bold">⚠️ <?= $error ?></div><?php endif; ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    <div class="bg-slate-900 border border-slate-800 p-8 rounded-2xl border border-slate-800 shadow-2xl shadow-lg flex flex-col items-center text-center hover:border-indigo-700/50 transition duration-300 group h-auto">
                        ...
                        <h2 class="text-2xl font-bold text-indigo-500 mb-2">Start Battle</h2>
    ...
    <div class="flex justify-between items-center mb-2">
        <h2 class="text-lg font-bold text-indigo-400"> NOTES</h2>
    ...
    <form method="POST">
        <textarea 
            ...
            class="w-full bg-slate-950 border border-slate-700 rounded p-3 text-sm text-indigo-200 font-mono mb-4 focus:ring-1 focus:ring-indigo-500 resize-none overflow-hidden"
        ...
        case 'wizard': 
            ...
            <div class="max-w-4xl mx-auto bg-slate-900 border border-slate-800 p-6 rounded-2xl border border-slate-800 shadow-2xl shadow-xl relative">
                <form method="POST" class="absolute top-4 right-4"><button type="submit" name="cancel_session" class="text-xs text-indigo-500 bg-indigo-900/10 hover:bg-indigo-900/30 px-3 py-1 rounded font-bold">✖ <?= t('cancel') ?></button></form>
                <?php if ($step === 1): ?>
                    <h2 class="text-xl font-bold text-indigo-500 mb-4"><?= t('step_1') ?></h2>
                ...
                <?php endif; ?>
                <?php if ($step === 2): ?>
                    <h2 class="text-xl font-bold text-indigo-500 mb-4"><?= t('step_2') ?></h2>
                ...
                <?php endif; ?>
                <?php if ($step === 3): ?>
                    <h2 class="text-xl font-bold text-indigo-500 mb-4"><?= t('step_3') ?></h2>
                    <form method="POST">
                            ...
                            <div class="bg-slate-950 p-3 rounded border border-slate-700 flex flex-col md:flex-row gap-4 loot-item-row">
                                <div class="w-full md:w-1/2">
                                    <div class="flex items-center gap-3 mb-2">
                                        ...
                                        <span class="font-bold text-indigo-100 item-name"><?= htmlspecialchars($it['name']) ?></span>
                                    </div>
                                    ...
                                    <div class="space-y-1 max-h-24 overflow-y-auto pr-1">
                                        <?php foreach($queueTop as $qp): $isPresent=in_array($qp['player_id'],$presentIds); $isBan = $qp['is_loot_banned']; $isExcl = $qp['is_excluded']; ?>
                                            <div class="flex justify-between <?= ($isBan || $isExcl) ? 'opacity-50' : '' ?>">
                                                ...
                                                <?= $isBan ? '<span class="text-[10px] text-indigo-500 ml-1">[BAN]</span>' : '' ?>
                                                <?= $isExcl ? '<span class="text-[10px] text-gray-500 ml-1">[EXCL]</span>' : '' ?>
                                                </span>
                                                <?= !$isPresent?'<span class="text-indigo-500 font-bold">ABSENT</span>':'<span class="text-green-500">●</span>' ?>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                </div>
        ...
        case 'database': ?>
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl border border-slate-800 shadow-2xl shadow-lg">
                    <h2 class="text-lg font-bold text-indigo-500 mb-4">Gracze</h2>
                    <form method="POST" class="flex flex-col sm:flex-row gap-2 mb-6">
                        <input type="text" name="new_player_nick" placeholder="Nowy Nick" class="flex-grow bg-slate-950 border border-slate-700 rounded p-2 text-white focus:border-indigo-600">
                            ...
                            <tr>
                                ...
                                <td class="py-2 px-2 text-right flex justify-end gap-1 items-center">
                                    ...
                                    <form method="POST" onsubmit="return confirm('<?= t('confirm_del_player') ?>')">
                                        ...
                                        <button type="submit" class="bg-indigo-900/20 hover:bg-indigo-900 text-indigo-500 hover:text-white text-[10px] font-bold px-2 py-1 rounded border border-indigo-900 ml-1">DEL</button>
                                    </form>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                ...
                <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl border border-slate-800 shadow-2xl shadow-lg flex flex-col h-full">
                    <h2 class="text-lg font-bold text-indigo-500 mb-4">Przedmioty</h2>
                    <form method="POST" class="space-y-2 mb-6">
                        <input type="text" name="new_item_name" placeholder="Nazwa" class="w-full bg-slate-950 border border-slate-700 rounded p-2 text-white focus:border-indigo-600">
                        <input type="text" name="new_item_icon" placeholder="Ikona (np. sword.png)" class="w-full bg-slate-950 border border-slate-700 rounded p-2 text-white focus:border-indigo-600">
                            ...
                            <tr>
                                ...
                                <td class="py-2 px-2 text-right flex justify-end gap-1 items-center">
                                    ...
                                    <form method="POST" onsubmit="return confirm('<?= t('confirm_del_item') ?>')">
                                        ...
                                        <button type="submit" class="bg-indigo-900/20 hover:bg-indigo-900 text-indigo-500 hover:text-white text-[10px] font-bold px-2 py-1 rounded border border-indigo-900">DEL</button>
                                    </form>
                                </td>
                            </tr>
                            <?php endforeach; ?>
            ...
            <div class="col-span-1 lg:col-span-2 bg-slate-900 border border-slate-800 p-6 rounded-2xl border border-slate-800 shadow-2xl shadow-lg mt-6">
                <h2 class="text-lg font-bold text-indigo-500 mb-4">Użytkownicy Czatu</h2>
        ...
        case 'history': 
            ...
            <div class="bg-slate-900 border border-slate-800 rounded-2xl border border-slate-800 shadow-2xl shadow-lg">
                <div class="p-4 border-b border-slate-800"><h2 class="text-2xl font-bold text-indigo-500 flex items-center gap-2">📜 Historia Sesji</h2></div>
                    ...
                    <table class="w-full text-sm text-left text-gray-400">
                        ...
                        <tbody class="divide-y divide-slate-800">
                            ...
                            <tr class="hover:bg-slate-800/50">
                                <td class="px-2 sm:px-6 py-4 font-bold text-indigo-500"><?= htmlspecialchars($s['name']) ?></td>
                                ...
                                <td class="px-2 sm:px-6 py-4 text-right flex justify-end gap-2 items-center">
                                    ...
                                    <form method="POST" onsubmit="return confirm('<?= t('confirm_delete') ?>')">
                                        ...
                                        <button type="submit" class="text-indigo-500 hover:text-indigo-400 font-bold px-1" title="Usuń">❌</button>
                                    </form>
                                </td>
                            </tr>
                            <?php endforeach; ?>
        ...
        case 'details': 
            ...
            <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl border border-slate-800 shadow-2xl shadow-lg">
                <div class="flex justify-between items-center border-b border-slate-800 pb-4 mb-4"><h2 class="text-2xl font-bold text-indigo-500">Podgląd Sesji: <?= htmlspecialchars($sess['name']) ?></h2><a href="<?= $self ?>?view=history" class="text-gray-400 hover:text-white">← Powrót</a></div>
        ...
        case 'edit': 
            ...
            <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl border border-slate-800 shadow-2xl shadow-lg">
                <div class="flex justify-between items-center border-b border-slate-800 pb-4 mb-4"><h2 class="text-2xl font-bold text-indigo-500">Edycja Sesji</h2><a href="<?= $self ?>?view=history" class="text-gray-400 hover:text-white">← Anuluj</a></div>
